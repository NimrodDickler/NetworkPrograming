...
{
	int command=htons(SESSION_ESTABLISHED);
	this->dispatcher = dis;
...
void Broker::sendMessage(TCPSocket * sender)
{
	int size, msg;
	char * temp;
	for(unsigned int i=0;i<this->socketListener.socketVector.size();i++)
	{
		if(this->socketListener.socketVector[i]->getSocketFid() != sender->getSocketFid())
		{
			msg=sender->recv((char*)&size,4);
			if(!sizeOfMessageCheck(msg))return; //validation
			size=ntohs(size);
			...
			if(!sizeOfMessageCheck(msg))return; //validation
			msg=this->socketListener.socketVector[i]->send(temp,ntohs(size));
			return;
		}
		else
		{
...
void Broker::close(bool closeConn, TCPSocket * closeSocket)
{
	...
	if(closeConn)
		this->dispatcher->openConnection(this->socketListener.socketVector,closeSocket);
	//stays open
	else
